...
void CeresFitting::GenerateData(const int &N) {
  for (int i = 0; i < N; i++) {
    ...
    double y = this->func(a_r, b_r, c_r, x) + rng.gaussian(w_sigma * w_sigma);
...
void CeresFitting::Draw() {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    ye_data.emplace_back(
        this->func(this->abc[0], this->abc[1], this->abc[2], i / 100.0));
  }
  plt::named_plot("estimated model", xe_data, ye_data, "r-");

  plt::title("Ceres Fitting Curve");
  plt::xlabel("x");
  plt::ylabel("y");
  plt::legend();
  plt::show();
